...
namespace Belediye_Otomasyonu.Views
{
    public partial class PersonelYonetimForm : Form
    {
        ...
        private void PersonelYonetimForm_Load(object sender, EventArgs e)
        {
            ...
            // Personel tabı
            UiTheme.DataGridStil(dgvPersonel);
            dgvPersonel.AutoGenerateColumns = false;
            dgvPersonel.Columns.Clear();
            dgvPersonel.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "Id", HeaderText = "ID", Width = 45 });
            dgvPersonel.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "KullaniciAdi", HeaderText = "Kullanıcı Adı", Width = 140 });
            dgvPersonel.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "Ad", HeaderText = "Ad", Width = 110 });
            dgvPersonel.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "Soyad", HeaderText = "Soyad", Width = 110 });
            dgvPersonel.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "Departman", HeaderText = "Departman", Width = 130 });
            ...
            // "Yönetici" onay kutusu etiketi güncelle
            if (chkYeniYonetici != null)
            {
                chkYeniYonetici.Text = "Departman:";
                chkYeniYonetici.Visible = false; // artık yönetici seçeneği yok — yöneticiler ayrı
            }

            // Departman etiket
            if (lblDepLabel != null)
                lblDepLabel.Visible = true;
            if (txtDepartman != null)
        ...
        {
            try { dgvPersonel.DataSource = BelediyeDbServisi.PersonelListesiGetir(); }
            catch (Exception ex) { MessageBox.Show("Liste yüklenemedi: " + ex.Message, "Veritabanı", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
        }

        private void btnListeYenile_Click(object sender, EventArgs e) => DoldurListe();

        private void btnVatandasYetki_Click(object sender, EventArgs e)
        {
            DataTable dt;
            try { dt = BelediyeDbServisi.KullanicilarPersoneleUygunGetir(); }
            catch (Exception ex) { MessageBox.Show(ex.Message, "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }

            if (dt.Rows.Count == 0)
            {
                MessageBox.Show("Uygun vatandaş bulunamadı.", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            using (var f = new Form())
            {
                ...
                var cb = new ComboBox { DropDownStyle = ComboBoxStyle.DropDownList, Dock = DockStyle.Top, Height = 28, DataSource = dt, DisplayMember = "KullaniciAdi", ValueMember = "KullaniciAdi" };
                ...
                panel.Controls.Add(iptal); panel.Controls.Add(tamam);
                f.Controls.Add(panel); f.Controls.Add(cb);
                f.CancelButton = iptal; f.AcceptButton = tamam;
                ...
                else { MessageBox.Show("Kullanıcı personele eklendi.", "Tamam", MessageBoxButtons.OK, MessageBoxIcon.Information); DoldurListe(); }
            }
        }
        ...
        {
            string dep = null;
            try { dep = txtDepartman?.Text; } catch { }
            ...
            else
            {
                ...
                txtYeniKadi.Clear(); txtYeniSifre.Clear();
                txtYeniAd.Clear(); txtYeniSoyad.Clear();
                if (txtDepartman != null) txtDepartman.Clear();
                DoldurListe();
            }
        }
        ...
        private void btnDurumDegistir_Click(object sender, EventArgs e)
        {
            ...
            var yeni = !simdiAktif;
            var msg = yeni ? "Seçili personeli aktifleştirmek istiyor musunuz?" : "Seçili personeli pasifleştirmek istiyor musunuz?";
            ...
            if (hata != null) MessageBox.Show(hata, "İşlem yapılamadı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            else DoldurListe();
        }
